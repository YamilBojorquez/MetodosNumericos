public class simulamrdxd {
    public static void main(String [] args) {
        ...
        int posicionCelda;
        for (int k = 0; k < N; k++) {
            exitJ: // Para usarse con un break para salir de los siguientes bucles una vez encontrada la celda del par que del que estamos buscando su celda
            for (int i = 1; i <= n+1; i++) {
                for (int j = 1; j <= n+1; j++) {
                    if (celdaEnX[k] == i && celdaEnY[k] == j) {
                        posicionCelda = (i - 1) * n + (j - 1); // Esto asegura que el índice esté dentro del rango
                        celda[posicionCelda]++; // Contamos que se encontró la celda a la que pertenece ese par
                        break exitJ;
                    }
                }
            }
        ...
        // Calculamos 
        double [] xpasadadeverga = new double[celda.length];
        double sumaxpasadadeverga = 0;
        for (int i = 0; i < celda.length; i++) {
            xpasadadeverga[i] = Math.pow(celda[i]-Eij, 2) / Eij;
            sumaxpasadadeverga += xpasadadeverga[i];
        }

        System.out.println(sumaxpasadadeverga);
